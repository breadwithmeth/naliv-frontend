...
export default function Item() {
    ...
    const fetchItem = async () => {
      try {
        ...
        const response = await fetch(
          createApiUrlWithParams(`/api/categories/items/${itemId}`, {
  ...
  const handleOptionsConfirm = (selectedOptions: SelectedOption[]) => {
    if (selectedBusiness && item) {
      ...
      // Логика умножения на parent_item_amount теперь находится в CartContext
      addItem({
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="bg-white rounded-lg p-4">
          ...
          {/* Кнопка добавления в корзину в основной части */}
          <div className="mb-4">
            <button
              onClick={() => {
                if (selectedBusiness) {
            ...
            >
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.5 5M7 13v6a2 2 0 002 2h6a2 2 0 002-2v-6M12 10v4m0 0l-2-2m2 2l2-2"
                />
              </svg>
              <span>
                {(() => {
                  ...
                  if (item.options && item.options.length > 0)
                    return 'Выбрать опции и добавить'
                  if (cartQuantity > 0)
                    return `Добавить ещё (в корзине: ${cartQuantity})`
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <button
                    ...
                    className="w-8 h-8 bg-white border border-orange-300 rounded-full flex items-center justify-center text-orange-600 hover:bg-orange-100"
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
                  ...
                  <span className="font-medium text-orange-800 min-w-[2rem] text-center">
                    {Math.floor(
                      cartQuantity /
                        (item.options && item.options.length > 0
                          ? Math.max(
                              ...item.options.flatMap(opt =>
                                opt.variants.map(v => v.parent_item_amount || 1)
                              )
                            )
                          : 1)
                    )}
                    ...
                    onClick={() => {
                      const multiplier =
                        item.options && item.options.length > 0
                          ...
                          : 1
                      const currentLogicalQuantity = Math.floor(
                        cartQuantity / multiplier
                      )
                      const newLogicalQuantity = currentLogicalQuantity + 1
                      const newActualQuantity = newLogicalQuantity * multiplier

                      if (newActualQuantity <= item.amount) {
                        updateQuantity(item.item_id, newLogicalQuantity)
                      }
                    }}
                    disabled={cartQuantity >= item.amount}
                    className={`w-8 h-8 border border-orange-300 rounded-full flex items-center justify-center ${
                ...
                <button
                  ...
                  className="text-xs text-orange-600 hover:text-orange-800 border border-orange-300 px-3 py-1 rounded-full hover:bg-orange-100"
                >
                  Удалить
                </button>
              </div>
